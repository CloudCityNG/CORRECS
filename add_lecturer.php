...
if(isset($_POST["register"])){
	...
	if(empty($error_array)){
		...
		if($execute){
			$register = $db->query("insert into lecturers values (?,?,?,?,?,?,?,?,?);", $find2[0]["(max(id) + 1)"], $fname, $lname, $onames, $uploaded_photograph, $_POST["dept"], $_POST["college"], $yearEmployed, $find[0]['(max(loginId) + 1)'], "insert" );
		if($register){
			$success_message =  "You have Created The new Lecturer<br />";
			$success_message .= "Go <a href = \"department_admin_page.php?department={$_POST["dept"]}\">Go Back to Department Admin Page</a>";
