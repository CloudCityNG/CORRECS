...
$this->AddPage('L','A4');
		...
		if(@$data[0][0] =="header"){
			...
			$this->Image('Pictures/Fountain.png',5,5,15,15,"");
			$this->Cell(280, 4, $data[0][1], 0, 0, 'C', false);
			 $this->Ln();
		 	$this->Cell(280, 4, $data[0][2], 0, 0, 'C', false);
		 	$this->Ln();
		 	
		 	$this->Cell(280, 4, $data[0][3], 0, 0, 'C', false);
		 	$this->Ln();
			$this->Ln();
		...
		$w = array(10,30, 58,40,35, 35, 35,20);
...
$i++;
		...
		*/
		...
		$header = array("s/n","Matric", "Semester Scores","Outstandings", "Previous", "Present", "Final", "Remarks");
		...
		$addHeader = true;
		$pdf->Header();
		//$pdf->Cell(11, 5, "ROXXOR_001", 0, 0, 'L');
		
		foreach($data as $row){
			$pagedata[$j] = $row;
			//var_dump($pagedata);
			if(($i%6)== 0 ||($i == count($data))){
